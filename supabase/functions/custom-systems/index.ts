...
const HEADERS = [
  ...
  "column_auto_now_json", "column_select_sheet_json", "column_select_parent_json",
  ...
  "ocr_model",
  // v20 additions (external AI provider selection: ''/'lovable' = Lovable gateway, 'google' = user's own Google AI Studio key):
  "ocr_provider",
...
function rowToSystem(r: Record<string, string>) {
  ...
  return {
    ...
    ocr_provider: clean(r.ocr_provider) === "google" ? "google" : "",
    ...
    teacher_match_mode: (["exact","contains","prefix","similarity","tokens"].includes(clean(r.teacher_match_mode)) ? clean(r.teacher_match_mode) : "exact"),
...
async function systemToRow(s: any): Promise<string[]> {
  ...
  const valByCol: Record<string, string> = {
    ...
    ocr_provider: String(s.ocr_provider || "") === "google" ? "google" : "",
    ...
    teacher_match_mode: (["exact","contains","prefix","similarity","tokens"].includes(String(s.teacher_match_mode || "")) ? String(s.teacher_match_mode) : "exact"),
